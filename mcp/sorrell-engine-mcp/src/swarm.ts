...
export interface IntentClassification {
  intent: "code_generation" | "ui_generation" | "data_analysis" | "full_stack" | "general";
  confidence: number;
  specialist: string;
  latencyMs: number;
}
...
export interface SwarmPlan {
  goal: string;
  steps: Array<{ step: number; action: string; tool: string }>;
  reasoning: string;
}
...
export class ModelSwarm {
  ...
  classifyIntent(input: string): IntentClassification {
    ...
    let intent: IntentClassification["intent"] = "general";
   ...
   */
  plan(goal: string): SwarmPlan {
    const steps: Array<{ step: number; action: string; tool: string }> = [];
   ...
   */
  buildApplication(description: string, framework: string = "vanilla"): GeneratedApp {
...
<script>${javascript}</script>
   ...
   */
  generateComponent(componentType: string, data?: string, framework: string = "html"): string {
...
<div class="sorrell-component" style="padding:1rem; border:1px solid #3b82f6; border-radius:6px; background:#0f172a; color:#f8fafc;">
   ...
   */
  analyzeData(rawData: string, question?: string): DataAnalysisResult {
    let rows: any[] = [];
    ...
    const numericSummary: Record<string, { min: number; max: number; mean: number; count: number }> = {};
   ...
   * Code Generator
   */
  writeCode(requirement: string, language: string = "typescript"): { language: string; code: string; tests: string } {
