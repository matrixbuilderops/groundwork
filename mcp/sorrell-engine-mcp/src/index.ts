...
async function startMcpServer() {
  ...
  server.tool(
    "classify_intent",
    "Sub-10ms intent classification via SecureBERT router. Determines whether a request requires UI, full-stack app, data analysis, or logic planning.",
    {
    ...
    async ({ input }) => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Error classifying intent: ${err.message}` }],
          isError: true
        };
      }
    }
  ...
  server.tool(
    "build_application",
    "Build a complete web application from a natural language prompt. Returns structured HTML, CSS, JavaScript, and a ready-to-render single-file bundle.",
    {
    ...
    async ({ description, framework }) => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Error building application: ${err.message}` }],
          isError: true
        };
      }
    }
  ...
  server.tool(
    "generate_component",
    "Generate a specific UI component (metric card, chart, form, navigation) with integrated styling and state bindings.",
    {
    ...
    async ({ componentType, data, framework }) => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: markup }]
        };
      } catch (err: any) {
        return {
          content: [{ type: "text" as const, text: `Error generating component: ${err.message}` }],
          isError: true
        };
      }
    }
  ...
  server.tool(
    "analyze_data",
    "Deterministically analyze CSV or JSON datasets: parses rows, computes min/max/mean distributions, and flags anomalous outlier records.",
    {
    ...
    async ({ data, question }) => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Error analyzing data: ${err.message}` }],
          isError: true
        };
      }
    }
  ...
  server.tool(
    "write_code",
    "Write production-grade code with accompanying test assertions for a given technical requirement.",
    {
    ...
    async ({ requirement, language }) => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Error generating code: ${err.message}` }],
          isError: true
        };
      }
    }
  ...
  server.tool(
    "swarm_plan",
    "Deconstruct high-level goals into deterministic sequential stages mapped to specialized model tools via Gemma-2B logic parser.",
    {
    ...
    async ({ goal }) => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Error planning goal: ${err.message}` }],
          isError: true
        };
      }
    }
