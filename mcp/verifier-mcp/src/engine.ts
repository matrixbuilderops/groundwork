...
export interface VerificationTier {
  ...
  name: string;
  threshold: string;
  status: "AUTO_APPROVED" | "STANDARD_REVIEW" | "EXPEDITED_REVIEW" | "CRITICAL_LOOP" | "REJECTED";
  description: string;
...
export interface InvoiceData {
  documentId?: string;
  subtotal?: InvoiceField | number | string;
  vat_amount?: InvoiceField | number | string;
  total_amount?: InvoiceField | number | string;
  invoice_date?: InvoiceField | string;
  payment_terms?: InvoiceField | string;
  customer_vat_id?: InvoiceField | string;
  vendor_address?: InvoiceField | string;
...
function parseFieldValue(v: any): number {
...
function getFieldString(v: any): string {
...
export function detectParadoxes(fields: InvoiceData): Paradox[] {
  const paradoxes: Paradox[] = [];

  // 1. Mathematical Paradox: Subtotal + VAT ≠ Total
  if (
    ("subtotal" in fields || "subtotal_amount" in fields) &&
  ...
  if (vatId.length >= 2 && address) {
    ...
    const euCountries: Record<string, string[]> = {
...
export function evaluateConfidenceTier(confidence: number): VerificationTier {
...
export function generateTruthToken(documentId: string, resolution: string, timestamp?: string): string {
  const ts = timestamp || new Date().toISOString();
  const payload = `${documentId}:${resolution}:${ts}`;
  return crypto.createHash("sha256").update(payload).digest("hex").slice(0, 16);
}

export function auditInvoice(invoiceData: InvoiceData, overrideConfidence?: number): AuditVerdict {
