...
import { detectParadoxes, evaluateConfidenceTier, generateTruthToken, auditInvoice } from "./engine.js";
const VERSION = "1.0.0";
...
async function startMcpServer() {
    ...
    server.tool("detect_paradoxes", "Detect logical impossibilities (mathematical discrepancies, temporal causality violations, identity mismatches) in extracted invoice fields.", {
        ...
        try {
            ...
            return {
                content: [{ type: "text", text: `Error detecting paradoxes: ${err.message}` }],
                isError: true
            };
        }
    });
    // Tool 2: evaluate_compliance_tier
    server.tool("evaluate_compliance_tier", "Route an extraction confidence score through the 5-tier EU e-invoice validation matrix (Level 1 Straight-Through to Level 5 Rejection).", {
        ...
        try {
            ...
            return {
                content: [{ type: "text", text: `Error evaluating tier: ${err.message}` }],
                isError: true
            };
        }
    });
    // Tool 3: generate_truth_token
    server.tool("generate_truth_token", "Emit a single-token cryptographic SHA-256 audit entry verifying that an invoice resolution occurred at an exact timestamp.", {
        ...
        try {
            ...
            return {
                content: [{ type: "text", text: `Error generating truth token: ${err.message}` }],
                isError: true
            };
        }
    });
    // Tool 4: audit_invoice
    server.tool("audit_invoice", "Perform a complete deterministic compliance audit on an invoice: detects paradoxes, applies confidence tiering, and emits an immutable truth token.", {
        ...
        try {
            ...
            return {
                content: [{ type: "text", text: `Error during invoice audit: ${err.message}` }],
                isError: true
            };
        }
    });
    // Tool 5: explain_paradox
    server.tool("explain_paradox", "Retrieve deterministic mitigation strategies for a given paradox type (MATHEMATICAL_PARADOX, TEMPORAL_PARADOX, IDENTITY_PARADOX).", {
